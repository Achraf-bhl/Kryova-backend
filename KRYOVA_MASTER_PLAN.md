...
| E6 | Solver federation (CalculiX) | **in progress (since 2026-09-05)** — **the current coding stretch; it opens gate G1** — 6.1's deck writer, 6.2's load/selector mapping and the `.frd` reader land; the `ccx` subprocess and the oracle comparison remain | `app/solve/calculix/deck.py`, `tests/test_solver_calculix.py` (29 tests, offline — writing a deck is string generation over arrays, so none of it needs the binary). **The loads are not re-derived**: `assemble_loads` already distributes a force over its region by tributary area, and the deck writes that same vector out as `*CLOAD` — which is what makes 6.5's oracle comparison mean anything, since a disagreement between the two solvers only localises if both were handed identical loads. Four traps that produce a deck CalculiX accepts and solves *wrongly*, each pinned by a test verified by breaking it: **numbering is 1-based** (an off-by-one shifts every load onto its neighbour and returns a plausible field); **a C3D10's midside nodes are not in our order** — gmsh's type-11 ordering and Abaqus's differ in the last two, so the permutation is `[0,1,2,3,5,4]` and getting it wrong gives a valid, solvable, differently-shaped element; **density leaves mm-N-MPa here** and must be written in tonne/mm³ (`kg/m³ × 1e-12`, steel's 7870 → 7.87e-9, the number an engineer recognises in an Abaqus deck) or gravity is out by twelve orders; and **a coordinate written at low precision is a different part**, so everything numeric goes out at `repr` grade. The permutation test checks what the permutation must *achieve* against both edge tables rather than asserting the constant equals itself. Decision 4 is honoured: GPL, so file/CLI boundary, never linked. **The `.frd` reader (`frd.py`, 20 tests) is documented-not-verified and says so in its own docstring** — the fixtures are built from the CalculiX manual, not captured from a real run, and a test written from the same documentation as the code can be wrong in the same direction. `describe()` exists so the first real `ccx` output reports every block it found and every record it could not classify, the way `catia_describe_dialog` names an unrecognised Win32 control class. Three things it gets right on purpose: records are read **by column, not by whitespace**, because `-1.23456E+00-2.34567E+00` is two numbers with no space between them and is ordinary in a file of negative stresses; stress components are matched **by declared name**, since CalculiX writes SXY SYZ SZX where Abaqus writes S12 S13 S23 and reading by position transposes the shears without erroring; and a node with no result is **refused rather than zeroed**, because zero is a plausible displacement for a node that was never solved. Von Mises is the hand-written solver's own function rather than a second copy — two implementations of one invariant can drift, and a drift there would read as a disagreement about the stresses. **Open decision, deliberately not made in the parser**: a `.frd` reports stress at *nodes* and `SolveOutput.von_mises` is per *element*, and averaging a node's neighbours smooths the peak a factor of safety comes from — `solver.py` will make that choice where it is visible. |
