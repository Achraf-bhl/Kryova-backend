...
| E15 | Throughput, storage, observability | **in progress (since 2026-09-06)** — the observability half only; storage and the compute fabric deliberately deferred. Timing and counting hooks for the spans that will hurt (gmsh's module lock, the CalculiX subprocess, kernel rebuilds, media IO), near-free when nobody is collecting. **The measurement has to exist before the thing being measured is built**, and a 5,000-part clash check and an optimisation loop are both landing now | `app/observe/` |
