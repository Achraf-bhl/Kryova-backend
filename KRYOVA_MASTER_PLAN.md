...
| E6 | Solver federation (CalculiX) | **in progress (since 2026-09-05); the current coding stretch, and it opens gate G1.** 6.1 complete end to end and **verified against the real program (2026-09-06)**: CalculiX 2.23 installed, a bar in tension gives σ = 25.000000 MPa against F/A to 5.7e-16 and δ within 4.3e-07 of FL/AE; `C3D10_MIDSIDE_ORDER` confirmed correct through real ccx; `frd.describe()` reports **zero unrecognised records**, so the parser is now measured rather than merely documented. **6.5 DONE** — the oracle agrees with the in-house solver to 4.4e-09 mm and 2.7e-13 MPa. **6.6 DONE** — the taxonomy's patterns confirmed verbatim against real output, nothing misclassified. 6.2's loads were never re-derived. Open: 6.3 (shells and beams — a frame meshed as solids is a mesh nobody can afford), 6.4 (modal/buckling decks written, blocked on one seam in `deck.py`), thermal in the deck, and the solver registry — until that lands **nothing in the product can choose CalculiX**. Three defects the real solver exposed are recorded and being fixed, the worst being that an **under-constrained model returns exit 0 and 5.4e+11 mm with no complaint** | `app/solve/calculix/{deck,frd,run,diagnose,solver}.py`, `app/solve/oracle.py`, `tests/test_solver_calculix_frd.py` (20), `tests/test_solver_oracle.py` (34+4 skipped) |
| E7 | V&V: NAFEMS, convergence, provenance | **in progress (2026-09-06) — code landed, TESTS NOT WRITTEN.** Convergence with Richardson extrapolation and an observed order, the NAFEMS cases, a provenance record following `kernel/provenance.py`, and 7.4's validation register as data. Unverified: the next session's first job here is the test file, not more code | `app/verify/` (1,563 lines, no tests) |
...
| E9 | Multibody (Chrono) | **in progress (2026-09-06) — code landed, TESTS NOT WRITTEN.** Kinematics and clearance through a motion range, which is what G3 needs first and needs no dynamics engine | `app/dynamics/` (2,927 lines, no tests) |
| E10 | Thermal, CFD, optimisation | partial — thermal *stress* shipped (2026-09-03); **optimisation in progress (since 2026-09-06)**, deliberately scoped away from CFD and conduction. This is the workload Decision 1 was made *for* — OCCT is the internal engine because a design loop needs tens of rebuilds a minute — and it gives `design/sensitivity.py` its first caller outside a test. A run that did not converge reports that, and never returns its last iterate | `app/solve/thermal.py` vs `σ=−EαΔT`; `app/optimise/`; conduction/CFD not started |
| E11 | Requirements model | **partial (2026-09-06), with tests.** — the phase that turns "a 200×150 plate at 2.4 kg" into "carries 5 kN at FoS 2 inside this envelope". Unblocks **E5.2**, which the board has recorded as blocked on exactly this, and is half of gate G2. Reuses `design/assertions.py`'s PASSED/FAILED/UNMEASURED and `kernel/contract.py`'s measurable vocabulary rather than forking either | `app/requirements/` |
| E12 | Load cases, materials, standard parts | **in progress (2026-09-06) — code landed, TESTS NOT WRITTEN.** — materials as records with provenance and condition, where a property nobody has a source for is *absent and says so*; an unknown material refused rather than silently substituted (a measured defect: an unrecognised slug quietly attached steel and the mass came back ~3× heavy); standard parts via BOLTS. The other half of gate G2 | `app/solve/materials.py`, `app/parts/` |
| E13 | Design rules, GD&T, cost | **in progress (2026-09-06) — code landed, TESTS NOT WRITTEN.** Tolerance stack-up worst-case and RSS with the conditions under which RSS is not legitimate; a rule engine reading `kernel/contract.py`'s vocabulary; GD&T as structured data. Cost deliberately out of scope — an uncalibrated cost model is a confident wrong number | `app/rules/` (1,159 lines, no tests) |
| E14 | Product structure & contracts | not started | |
| E15 | Throughput, storage, observability | not started | |
| E16 | Tool retrieval, planning, memory | partial — resume-from-log shipped (2026-09-03); **16.1 now has a measurement, and it is the bottleneck** (2026-09-05) | `app/ai/resume.py`, `tests/test_resume.py`; retrieval/planning open. **Measured on this seat with `qwen3-coder:30b` against the 108-tool payload**: asked for a mounting flange (100 mm plate, central bore, four holes on a bolt circle, corner fillets — six features that must agree), the model opened with `catia_pad` on a conversation with no document, invented the argument `profile`, and invented two tools that have never existed (`catia_circle`, `catia_pattern_circle`). It never called `catia_sketch_create`. **Every one of those was refused by name and no wrong geometry was built — the validation layer worked exactly as designed** — but nothing was built at all. The three places it could have been our fault were each checked and were each correct: the CATIA prompt variant is selected (`AGENT_SYSTEM_CATIA_DOCS`), it now teaches the four-step modelling order explicitly, and the per-turn brief already said *"Call catia_new_part ... before any other geometry operation"* right beside the user's message. Told twice, in the two places built for it, and it still opened with a pad. Adding the modelling order to the prompt moved exactly one thing — `depth_mm` became `length_mm` — and changed nothing else. So this is agent capability, not plumbing, and it is what 16.1/16.2 exist for: 108 schemas are ~16k prompt tokens re-evaluated every turn (`prompt_eval_cached_count: 0`) and the model cannot hold a six-feature intent across them. **Part 5's first "fact to keep in view" is now observed rather than anticipated.** **16.1 then landed the same night and moved the needle** (`app/ai/tool_retrieval.py`, `tests/test_tool_retrieval.py`, 22 tests): with `AI_TOOL_LIMIT=40` the *same* rung-2 request that had produced nothing twice built the whole flange — new part, sketch, rectangle, pad, bore sketch, circle, pocket, hole sketch, four circles, pocket, fillet, material, measure. **The load-bearing design decision is that retrieval narrows what the model is *shown*, never what it can *call*** (`ToolBox.schemas(only=)` filters; `ToolBox.call` is untouched), so no setting can make a capability unreachable and the worst case is a turn where the model names a tool from memory — which still works. The core modelling loop is never withheld whatever the query says, and recently-used tools stay offered because continuity beats similarity. **And the result is the strongest possible argument for Decision 3**: every tool call succeeded and the part is still *wrong*. The holes went to (±35, ±35) — a bolt-circle radius of 49.5 mm where a 70 mm bolt circle means radius 35, the model having read the number as a coordinate; and `edges='all'` rounded every edge including the bore and hole rims rather than the four vertical corners, which is the 4,494 mm³ the measured 97,208 falls short of the closed-form 101,702. It renders as an entirely plausible flange. **Mechanical completion is now the easy half; nothing in the chat path yet checks the part against what was asked**, and that is what E5's assertions and 5.1's machine checks exist to do — wiring them into the conversation is the next thing 16.x owes. **Done the same night: `check_part`** — the agent measures the part it built and checks one claim per requirement through `assertions.check_assertions`, unchanged, so `UNMEASURED` keeps its meaning and is reported in words as *not* a pass. Nothing is re-implemented: the measurement is the `catia_measure` both backends already answer, and the tool reads either payload shape so a caller need not know which one did. It is in `CORE_TOOLS`, so retrieval can never withhold the check and produce a part nobody verified — the failure 16.1 would otherwise have caused while fixing a different one. 25 tests, including the flange as it was really built: a volume claim catches the 4,494 mm³ that rounding every edge removed. **Driven live, and the model used it** — refused three times for an `==` with no tolerance, it adapted to `>=`/`<=` pairs and passed six claims. But it chose width, height and thickness, **all three bounding-box readings, and a bounding box is unchanged by every internal feature** — so it passed on a part whose bore, holes and fillets were never checked. `check_part` now says so: a claim set reading neither `volume_mm3` nor `mass_kg` is flagged `blind_to_features`, because those two are the quantities that move when any feature does. A caveat and not a refusal, since checking one dimension mid-build is legitimate and refusing it would teach the model to stop calling the tool at all. **Also found by running it**: `ai_daily_token_budget` was read with `getattr(settings, ...)` and was not a field on `Settings`, so with `extra="ignore"` the documented `AI_DAILY_TOKEN_BUDGET` was accepted, dropped, and the knob did nothing — the same trap `max_steps()` documents and works around, which the budget had without the workaround. And a six-feature part with retries needs more than the 20-step default: it ran out mid-fillet before reaching the check. **And the last piece of the flange was not a model failure at all — it was a vocabulary gap** (`app/catia/ops/placement.py`, `tests/test_placement.py`, 43 tests). `catia_sketch_circle` offered exactly one way to say where a circle goes, Cartesian `at`, while the prompt forbids the model from doing coordinate arithmetic on the grounds that it belongs inside a tool where it can be tested. "Four holes on a 70 mm bolt circle, one in each corner direction" is radius 35 at 45°, which is (24.749, 24.749) — so the tool *required* the one thing the prompt forbade, and the model did the only other thing available and used the radius as a coordinate. Told not to, it still had to. `at_radius_mm`/`at_angle_deg` are now declared on circle, rectangle and polygon, and the trigonometry happens once, on the server, in `dispatch._augment`: they are `consumed_by_server`, so the workstation daemon never sees a polar argument and `generated_tools.py` regenerated byte-identical — and, more to the point, **there is one implementation and therefore one angle convention** (anticlockwise from the sketch's horizontal axis, which is `catia_sketch_arc`'s). `OcctRunner` calls the same function directly, because the design IR and the mission ladder never go through the dispatcher and a spec that builds through the chat but not through a sweep would be the worst of both. Dispatch asks the *schema* whether a tool offers polar placement rather than holding a list of tool names, so adding `*polar_placement()` to a new operation is the whole of the change. The prompt gained one general rule and no recipe: a position stated as an offset is `at`, a position stated as a distance and a direction is polar, and turning the second into the first yourself *is* the coordinate maths. **Rung 2 then passed cleanly on the same night, and the part is right** (`docs/verification-2026-09-05-night/REPORT.md`): 18 calls, no refusals, no retries, volume and mass matching the closed form to the last digit and a feature list naming every feature built. Reaching that took three more defects out of our own code, and **not one of them was visible from a green suite — all three were found by driving the product**, which is the whole argument for the standing end-to-end rule. `PartDocument.measure` cached its payload against the shape and `catia_set_material` changes the density and no geometry, so a material set after the first measurement never reached a mass again — the agent was handed a material name, no `mass_kg`, and `mass_is_provisional`, and *invented* 273 kg for a part weighing 0.27. Fabricating it was the model; being unable to read one was us. `Assertion` refused `solid_count == 1` for want of a tolerance, which is right for a length and wrong for a count, and cost three of the turn's steps teaching the model that our own verification tool does not mean what it says — `counts_things()` decides from the path, with a test reading the real contract so the convention cannot drift from `unit="count"` without the design package importing 166 MB of OCP. And every operation defaulted an unnamed feature to its tool's own word, so a second pocket arrived at `add_feature` as `pocket` and was treated as a *regeneration of the first* — correct behaviour for a rebuilt plan, catastrophic for two distinct features: the flange reported three features for four operations, and a fillet scoped to `Pocket.1` would have reached the holes. **All three left the geometry correct, which is exactly why they survived four verification runs.** Not verified on a CATIA seat this session: the bridge daemon is not running and binding it to the overnight test account unattended is not a trade worth making; what can honestly be said without it — polar placement is server-consumed and the daemon schema regenerated byte-identical, and the other two fixes are in `app/kernel/occt/`, which a seat does not execute — is recorded as an argument and not as a measurement. **Rung 3 attempted 2026-09-06 and not passed — reported as the rung reached, not as a pass.** Three defects came out of it, all ours, and the third is the one worth keeping in view: `catia_list_parameters` handed the agent a name it could not type back. The payload is JSON, so a name containing a backslash is *shown* with that backslash escaped, the model typed what it read, and all four of its `catia_set_parameter` calls were refused for punctuation. It then abandoned the loop, padded a slab over the part, and reached 2.401 kg — **inside the 20-gram tolerance it was given, with the bore and all four corner holes filled in.** A part that weighed exactly what was asked for and was not the part that was asked for, which is the cleanest illustration this project has produced of why a number is not a verification (`454d780` folds every separator to one key, and accepts a bare unambiguous dimension name). Also observed once and not a defect: `qwen3-coder:30b` emitted a textual `<function=...>` block instead of a `tool_calls` payload, and the agent layer said so in words — *"nothing has actually been done"* — rather than narrating work it had not done. |
| E17 | Manufacturing output | **in progress (2026-09-06) — code landed, TESTS NOT WRITTEN.** — drawings built on `app/render/`'s existing deterministic hidden-line engine rather than a new one, DXF via `ezdxf`, and dimensions traced to the *parameter* that set them (which `app/design/` knows and most CAD systems do not). A drawing that could not be fully dimensioned says so, in the drawing | `app/manufacture/` |
...
| P4 | File attachments & understanding | partial — chunked upload + content-addressed store exist; **the understanding layer landed 2026-09-06 with TESTS NOT WRITTEN**, including Decision 8's injection boundary built structurally rather than as a filter. The injection *attack* tests are the ones that matter and they do not exist yet | `app/media/`, `app/documents/` (924 lines, no tests) |
