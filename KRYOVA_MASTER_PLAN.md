...
| — | **Integration gap (not a phase)** | **open — found 2026-09-05** | `app/catia/dispatch.py` has no backend selection; `OcctRunner` is built only in `app/kernel/` and tests | **Era I–II is green on capability and connected to nothing.** The agent can only drive CATIA, so 108 kernel operations need a licence to reach; `app/render/`, `app/ai/vision.py`, `machine_checks.py` and `sensitivity.py` have no caller outside tests. Decision 1 says the IR compiles to an open kernel *first* and CATIA is one backend among several — today CATIA is the only one dispatch can reach. Queued ahead of E6. |
