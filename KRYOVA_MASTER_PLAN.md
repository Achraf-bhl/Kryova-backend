...
| E7 | V&V: NAFEMS, convergence, provenance | **in progress (since 2026-09-06)** — convergence study with Richardson extrapolation and observed order, the NAFEMS cases a linear-static/modal solver can be held to, the provenance record following `kernel/provenance.py` rather than a second vocabulary, and 7.4's validation register as data | `app/verify/{convergence,benchmarks,provenance,quantities}.py` |
| E8 | Fatigue (pyLife) | **in progress (since 2026-09-06)** — rainflow and damage accumulation federated to pyLife per Decision 2, with the load-history / factor / provenance vocabulary kept ours. Verified against closed form (exact cycle count, Miner arithmetic), and every factor a qualified engineer must choose is an explicit input with a source field rather than a buried default | `app/fatigue/` |
| E9 | Multibody (Chrono) | **in progress (since 2026-09-06)** — the phase that says where load cases come from: mechanism output lands in the *existing* `LoadCase` vocabulary, not beside it. Kinematics and clearance-through-travel are buildable and checkable against closed form with no dynamics engine, which is what gate G3 needs first | `app/dynamics/` |
...
| E11 | Requirements model | **in progress (since 2026-09-06)** — the phase that turns "a 200×150 plate at 2.4 kg" into "carries 5 kN at FoS 2 inside this envelope". Unblocks **E5.2**, which the board has recorded as blocked on exactly this, and is half of gate G2. Reuses `design/assertions.py`'s PASSED/FAILED/UNMEASURED and `kernel/contract.py`'s measurable vocabulary rather than forking either | `app/requirements/` |
| E12 | Load cases, materials, standard parts | **in progress (since 2026-09-06)** — materials as records with provenance and condition, where a property nobody has a source for is *absent and says so*; an unknown material refused rather than silently substituted (a measured defect: an unrecognised slug quietly attached steel and the mass came back ~3× heavy); standard parts via BOLTS. The other half of gate G2 | `app/solve/materials.py`, `app/parts/` |
