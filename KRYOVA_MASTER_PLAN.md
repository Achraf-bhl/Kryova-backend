...
| E18 | Machine missions M1–M8 | partial — **M1 green (2026-09-05) and M2 green (2026-09-06), the first rung that is a *product* rather than a part**; M3–M9 declared and PENDING | `app/design/missions.py` `LADDER`, `tests/test_design_missions.py` (32), `tests/test_mission_m2.py` (48). M1 builds on the real kernel and holds eight closed-form claims. M2 is a welded portal frame — two components, three occurrences, one interface contract carrying the section both members are built from (14.3), a clash check that measures both joints, a mass roll-up against a closed form, and a world envelope; 18 mission assertions and 6 interface claims, all measured, none `UNMEASURED`. Six wrong frames are built in the tests and each named guard is watched to fail. **M2 passes with `unproven` attached**: the weld is not sized (E6), not classified (E8.3) and there is no weldment or cut list (E17.4) — so `LadderReport.complete` is False while a caveat stands, and “M2 passes” cannot be read as “the welds are sized”. Coverage is measured (2/9) rather than asserted. |
