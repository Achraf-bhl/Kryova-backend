...
| E7 | V&V: NAFEMS, convergence, provenance | **in progress — convergence, the benchmark honesty rules and the provenance chain now have tests (2026-09-06)**. A `PUBLISHED` target needs a citation, an `UNKNOWN` one forbids a value, a tolerance must be justified, and exactly one `Outcome` is a pass. The mesh digest is pinned per input — node coordinates, connectivity and midside nodes each tested in isolation, because refining or promoting a mesh changes several at once and a test that moves two things proves nothing about either. The validation register is still `TESTS NOT WRITTEN`, and **no NAFEMS cases are encoded yet**, which the module says of itself | `app/verify/`, `tests/test_verify_{convergence,benchmarks,provenance}.py` |
...
| E14 | Product structure & contracts | **in progress (since 2026-09-06)** — the phase gate **G3** needs, and the one the mission ladder cannot pass rung 3 without: M2 onwards are all assemblies. A product structure that is a *graph*, not a tree of copies (a bolt used 40 times is one part and 40 occurrences); interface contracts as assertions over a boundary, naming which side a change violated; a clash check honest about which pairs it actually examined | `app/assembly/` |
| E15 | Throughput, storage, observability | **partial — the observability half (15.5) DONE (2026-09-06)**; 15.1–15.4 (batch compilation, autoscale, result caching, the CATIA session pool) not started and deliberately deferred. `~0.09 µs` per disabled span, so it can be left in. The catalogue lists every span the system *could* produce **including the ones nothing emits yet**, with a required reason — `contract.py` applied to timings, and what lets a report name a hole in the instrument rather than omit it. Coverage reuses `assertions.Outcome`: `UNMEASURED` for all four ways a site produced nothing, and `FAILED` never appears there because a span that raised is a *successful measurement of a failure* | `app/observe/` (5 modules), `tests/test_observe*.py` (68) |
| E16 | Tool retrieval, planning, memory | partial — resume-from-log shipped (2026-09-03); **16.1 in progress (2026-09-06) and it is the bottleneck**. The OCCT backend offers **108 tools** and `AI_TOOL_LIMIT` is 40, so 68 are invisible to the agent. The prize is measured: `qwen3.5:9b` fits entirely in this machine's 8 GB of VRAM and *failed* the 40-tool payload where the 30B model running 72% on the CPU succeeded — **shrink the payload and a far faster model becomes usable**, which changes the economics of every gate run | `app/ai/resume.py`, `app/ai/retrieval.py`, `docs/verification-2026-09-06/REPORT.md` |
...
| E17.3 | Sheet metal (pulled forward to Era IV) | **in progress (since 2026-09-06)** — bend allowance `BA=(π/180)·θ·(r+K·t)`, setback and deduction, unfold, and the formability rules, each checked against arithmetic rather than recorded output. The K-factor is an **input with a source**, never a buried default — the same rule `app/parts/` applies to a proof load and `stackup.py` to an RSS number | `app/sheetmetal/` |
...
| P2 | Orgs, roles, RLS tenancy | **in progress (since 2026-09-06)** — orgs own projects, roles in one place rather than scattered through routes, and **RLS as the safety net under application scoping**, fed only by `SET LOCAL` inside an explicit transaction (safe on a pooled endpoint because it dies at COMMIT, which plain `SET` does not — that has already bitten this repo once). Cross-tenant is 404. The migration must backfill an owning org for every existing project | `app/models/organisation.py`, `app/api/routes/organisations.py` |
