...
| E7 | V&V: NAFEMS, convergence, provenance | **in progress — convergence and the benchmark honesty rules now have tests (2026-09-06)**. The rules are structural: a `PUBLISHED` target requires a citation, an `UNKNOWN` one **forbids a value** and must say what would make it known, a tolerance must be *justified* (an unjustified band is one that gets widened the first time a run misses it), and **exactly one `Outcome` is a pass** — `MEASURED` and `UNCONVERGED` are honest results and are not validation. `provenance.py` and the validation register still `TESTS NOT WRITTEN`; no NAFEMS cases are encoded yet, which the module says of itself | `app/verify/`, `tests/test_verify_convergence.py`, `tests/test_verify_benchmarks.py` (14) |
...
| E9 | Multibody (Chrono) | **in progress — clearance through a motion range now has tests (2026-09-06)**, which is what gate G3 needs first and what needs no dynamics engine at all: a collision in the *middle* of the travel is caught (the case an endpoint check misses), and an unlooked-at pose is never counted as a measured one. The rest of `app/dynamics/` — kinematics, reactions, load-case output — is still `TESTS NOT WRITTEN` | `app/dynamics/`, `tests/test_dynamics_clearance.py` |
| E10 | Thermal, CFD, optimisation | partial — thermal *stress* shipped (2026-09-03); **optimisation's honesty rules now have tests (2026-09-06)**: an optimum ON an active constraint is not a failure, and a run that did not converge never returns its last iterate. OpenMDAO installs on Python 3.14. The rest of `app/optimise/` is still `TESTS NOT WRITTEN`; conduction/CFD not started | `app/solve/thermal.py` vs `σ=−EαΔT`; `app/optimise/`, `tests/test_optimise_honesty.py` |
...
| E12 | Load cases, materials, standard parts | **in progress — materials and standard parts now have tests (2026-09-06)**. Bolt data checked against **ISO 898-1 arithmetic** rather than the record (proof load = stress area × 580 MPa for an 8.8), and every absent quantity carries a *reason*: tightening torque is missing from every bolt because it is a property of the **joint**, not the bolt — friction varies by a factor of three between dry and lubricated — so it is computed from VDI 2230 with a coefficient the caller must defend. `load_library.py` still `TESTS NOT WRITTEN` | `app/solve/materials.py`, `app/parts/`, `tests/test_materials.py`, `tests/test_parts_catalogue.py` (17) |
| E13 | Design rules, GD&T, cost | **in progress — tolerance stack-up now has tests (2026-09-06)**, checked against arithmetic done in the test rather than against recorded output. The judgement half is the valuable part and it is enforced: **an anonymous RSS number is not obtainable from `stack()`** — every assumption in `Risk` must be established from declared capability data or explicitly acknowledged *by name*, and `Risk.INDEPENDENCE` can never be established from numbers. `engine.py` and `gdt.py` still `TESTS NOT WRITTEN`; cost deliberately out of scope | `app/rules/`, `tests/test_rules_stackup.py` |
...
| E17 | Manufacturing output | **in progress — the projection convention and the honesty block now have tests (2026-09-06)**. First and third angle demonstrably place views on opposite sides, so a convention that were stored and ignored is caught: swapping them fails two tests, and a drawing read in the wrong convention is manufactured **mirrored** with nothing looking wrong. A dimension traced to its parameter is distinguishable from one measured off the solid. Dimensioning, sections, DXF and STEP export still `TESTS NOT WRITTEN` | `app/manufacture/`, `tests/test_manufacture_drawing.py` (12) |
