...
def _validate_object(value: dict[str, Any], schema: dict[str, Any], path: str) -> None:
    ...
    for name in schema.get("required", []):
        if name not in value:
            ...
            # The accepted shape goes here too. A model that omitted one
            # required field has usually misremembered the whole signature, and
            # telling it about this field alone sends it round again for the
            # next one -- which is exactly the three-turn loop this file's
            # `describe_properties` docstring records.
            raise SchemaError(
