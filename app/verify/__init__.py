"""Verification: whether a number is the model's answer or the mesh's — and never validation.

Master plan Phase 7. The modules below, and the split is the ASME VVUQ 1 split —
`app.verify.standards` names the document behind every standards claim this
package makes, including why the terminology split is VVUQ 1's and not
V&V 20-2009's despite V&V 20 being cited elsewhere here for its own method:
...
app/verify/
  ...
  standards.py     20.1/20.3 — which ASME document is behind each word, quoted
  corpora.py       20.4 — the solvers' own test corpora on pinned builds, and what
                   a green run of each does and does not establish
...
* 7.1 has its machinery (`benchmarks.py`) and, since 2026-09-08, a catalogue
  ...
  square plate — and it agrees, converged over three grids, 1.06% from the
...
* 7.4's published verification register is `register.py`, and as of 2026-09-08 it
  said **1 of 11 analyses agree with a benchmark** — modal, against FV52 — where it had said
  ...
  (`data/verify/benchmark-outcomes.json`), because a benchmark case is several
  ...
  mismatch takes the page back to "nothing agrees" with the reason stated.
