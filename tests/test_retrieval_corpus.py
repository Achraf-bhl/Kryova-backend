...
precision@3 100%, MRR 0.974. Re-measured on the Windows seat 2026-09-06 and
identical, which is the point of writing the number down.

**The measurement is printed, not only asserted.** These three numbers are the
claim the rest of the retrieval work rests on, and a claim that only appears in
an assertion message is a claim nobody reads until it has already broken. The
thresholds below are floors; the line `test_precision_and_mrr_hold` writes to
the terminal is the measurement.

A note for whoever finds this file thin again. It was replaced by a 49-line
stub on 2026-09-02 -- nine `assert the right document is somewhere in the top
five` cases, no scoring, no corpus health, no citation health -- twenty-six
minutes after it first landed, in a commit whose message says it *added* corpus
tests. Nothing failed, because a suite that measures less always passes more.
The index the stub was watching then drifted for three days without anybody
noticing that precision@1 had fallen 2.6 points. If a change here makes the file
shorter, that is the thing to be suspicious of.
"""
...
def announce(config: pytest.Config, line: str) -> None:
    """Put `line` on the terminal whether or not the test failed.

    `print` is captured, so a measurement printed normally is visible only when
    the assertion below it has already fired -- which is the one moment it is
    too late to be useful. The terminal reporter writes past capture. It is a
    plugin lookup rather than an import because a bare `pytest` run with a
    different reporter (or none, under `-p no:terminal`) must not fail here for
    the sake of a status line.
    """
...
class TestRetrievalQuality:
    ...
    def test_precision_and_mrr_hold(
        self, service: KnowledgeService, passages: list[dict], pytestconfig: pytest.Config
    ):
        ...
        announce(
            pytestconfig,
            f"\n  retrieval over {len(passages):,} real passages: "
            f"precision@1 {precision_1:.1%}, precision@3 {precision_3:.1%}, "
            f"MRR {mrr:.3f} over {total} cases "
            f"(floors {MIN_PRECISION_AT_1:.0%} / {MIN_PRECISION_AT_3:.0%} / {MIN_MRR:.2f})",
        )
    ...
    def test_a_multi_term_query_returns_only_broad_matches(self, index_dir: Path):
        """The other half of the coverage floor, and the half with a cost.

        The test below proves a query about nothing comes back empty. This one
        proves the floor is doing that by *breadth* rather than by score: every
        hit for a four-term question matches at least two of them, so a page
        that merely repeats `angle` forty times cannot buy its way in.

        Measured on the corpus directly rather than through the service,
        because `KnowledgeService.search` widens the query first and the floor
        is then measured against the user's original -- which is the behaviour
        `Corpus.search(coverage_query=)` exists for and is tested for
        separately. Here the question is the floor itself.
        """
        corpus = Corpus.open(index_dir)
        assert corpus is not None
        hits = corpus.search("draft angle neutral element", limit=5)
        assert hits, "the corpus does cover this"
        for hit in hits:
            assert hit.matched_terms >= 2, f"{hit.citation()} matched only one term"
